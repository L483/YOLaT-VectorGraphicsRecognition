    ...
    with torch.no_grad():
        ...
        for i_batch, (data, slices) in enumerate(test_loader):
            ...
            for i in range(0, len(image_slice) - 1):
                ...
                print(pred_coord_img.cpu().numpy().shape, np.ones(
                    (pred_coord_img.size(0), 1)).shape, pred_label.cpu().numpy()[:, None].shape)
                detections = [np.concatenate([pred_coord_img.cpu().numpy(),
                                              np.ones(
                                                  [pred_coord_img.size(0), 1]),
                                              np.ones(
                                                  [pred_coord_img.size(0), 1]),
                ...
                print("\t+ Batch %d-%d, Inference Time: %s" %
                      (i_batch, i, inference_time))
