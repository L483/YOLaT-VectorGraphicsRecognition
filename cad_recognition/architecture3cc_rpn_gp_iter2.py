...
class Backbone(torch.nn.Module):
    ...
    def forward(self, x, edges, edge_weights, edge_attrs, bbox_idx):
        f, f_super = self.head(
            x, edges[0], edge_weights[0], edge_attrs[0], x_node=x)
        ...
        for i in range(self.n_blocks-1):
            ...
            f, f_super = self.backbone[i](
                f, edges[0], edge_weights[0], edge_attrs[0], x_node=f_super)
            feats.append(f)
            feats_super.append(f_super)

        feats = [feats[i]
                 for i in range(self.n_blocks - self.n_blocks_out, self.n_blocks)]
        feats = torch.cat(feats, dim=1)
        fusion_feats = self.fusion_block(feats)
        out_feat = torch.cat((fusion_feats, feats), dim=1)

        feats_super = [feats_super[i] for i in range(
            self.n_blocks - self.n_blocks_out, self.n_blocks)]
...
class SparseCADGCN(torch.nn.Module):
    def __init__(self, opt, n_edges=3, edge_max_pool=torch.nn.AdaptiveAvgPool1d, expand_ratio=0.25):
        ...
        self.prediction_cls = MultiSeq(*[MLP([(self.cls_net.fusion_dims + 1024) * 2 + self.dim_stat, 512], act, norm, bias),
                                         MLP([512, 256], act, norm,
                                             bias, drop=opt.dropout),
    ...
    def forward(self, data, slices):
        ...
        out_feat_cls, out_feat_cls_super = self.cls_net(
            x, edges, edge_weights, edge_attrs, bbox_idx)
    ...
    def predict(self, data, slices):
        ...
        for i in range(0, len(slice_root) - 1):
            ...
            for root in sub_roots:
                slice_pos += list(range(root.value['idx_pos'][0] + slices['pos']
                                  [i], root.value['idx_pos'][1] + slices['pos'][i]))
                slice_edge += list(range(root.value['idx_edge'][0] + slices['edge']
                                   [i], root.value['idx_edge'][1] + slices['edge'][i]))
                slice_edge_super += list(range(root.value['idx_edge_super'][0] + slices['edge_super']
                                         [i], root.value['idx_edge_super'][1] + slices['edge_super'][i]))
                slice_bbox.append(
                    int(root.value['idx_bbox'] + slices['bbox'][i]))
        ...
        def build_data(data, slice_pos, slice_edge, slice_edge_super, slice_bbox):
            ...
            new_data.bbox_idx = torch.tensor(
                np.array(new_bbox_idx), dtype=torch.long)
            # torch.cuda.synchronize()
            # et = time.time()
        ...
        # print(is_object)
        # (has_object == 1) & (is_object == self.n_classes - 1)
        has_object = (is_object == self.n_classes - 1)
        ...
        for i in range(0, len(slice_root) - 1):
            ...
            for root in sub_roots:
                ...
                for child in root.children:
                    slice_pos += list(range(child.value['idx_pos'][0] + slices['pos']
                                      [i], child.value['idx_pos'][1] + slices['pos'][i]))
                    slice_edge += list(range(child.value['idx_edge'][0] + slices['edge']
                                       [i], child.value['idx_edge'][1] + slices['edge'][i]))
                    slice_edge_super += list(range(child.value['idx_edge_super'][0] + slices['edge_super']
                                             [i], child.value['idx_edge_super'][1] + slices['edge_super'][i]))
                    slice_bbox.append(
                        int(child.value['idx_bbox'] + slices['bbox'][i]))
                count += 1
        ...
        if len(slice_pos) == 0:
            ...
            # st = time.time()
            new_data = build_data(
                data, slice_pos, slice_edge, slice_edge_super, slice_bbox)
            # torch.cuda.synchronize()
            # et = time.time()
            ...
            def interleaf_pc(slice_p, slice_c, out_p, out_c):
                ...
                for i in range(len(slice_c) - 1):
                    ...
                    s.append(s[-1] + end_root - start_root +
                             end_child - start_child)
                return out, s

            pred_cls, slice_image_bbox = interleaf_pc(
                slice_image_bbox_root, slice_image_bbox_child, pred_cls, pred_cls2)
            pred_bbox, slice_image_bbox = interleaf_pc(
                slice_image_bbox_root, slice_image_bbox_child, pred_bbox, pred_bbox2)
            slice_bbox, slice_image_bbox = interleaf_pc(
                slice_image_bbox_root, slice_image_bbox_child, torch.tensor(slice_bbox_root), torch.tensor(slice_bbox))
        ...
        x1 = center_x + w / 2
        y1 = center_y + h / 2
        pred_bbox = torch.cat([x0.unsqueeze(1), y0.unsqueeze(
...
class DetectionLoss(torch.nn.Module):
    ...
    def forward(self, out, data):
        ...
        if self.classifier != 'softmax':
            gt_cls = torch.zeros(pred_cls.size()).cuda().scatter_(
                1, gt_cls.unsqueeze(1), 1)
