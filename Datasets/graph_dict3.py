...
class SESYDFloorPlan(torch.utils.data.Dataset):
    def __init__(self, root, opt,
        ...
        self.attr_mean = np.array(
            [stats['angles']['mean'], stats['distances']['mean']])
        self.attr_std = np.array(
            [stats['angles']['std'], stats['distances']['std']])
    ...
    def random_transfer(self, pos, bbox, gt_bbox, bbox_targets):
        ...
        bbox_targets = self.__transform_bbox__(
            bbox_targets, scale, angle, translate)
    ...
    def _get_proposal(self, graph_dict, gt_bbox, gt_labels, bbox_sampling_step=-1):
        ...
        if self.do_mixup:
            cc, pos, edge, edge_super, e_attr, e_attr_super, is_super = self.mixup(
                cc, pos, edge, edge_super, e_attr, e_attr_super, is_super)
        ...
        for cc_idx, cluster in enumerate(cc):
                    ...
                    for grid_y1 in y_grids[i_grid_y0 + 1:]:
                        ...
                        for grid_x1 in x_grids[i_grid_x0 + 1:]:
                            ...
                            if use_bit:
                                ...
                                if x0 > 0 and y0 > 0:
                                    dd = d00[y1][x1].difference(
                                        d00[y1][x0 - 1]).difference(d00[y0 - 1][x1])
                                elif x0 > 0 and y0 == 0:
                                    dd = d00[y1][x1].difference(
                                        d00[y1][x0 - 1])
                                elif y0 > 0 and x0 == 0:
            ...
            for idxs in sub_clusters:
                ...
                # print(edge_bbox)
                edge_bbox = np.array(
                    [[o2n[e[0]] + offset, o2n[e[1]] + offset] for e in edge_bbox])
                ...
                edge_super_bbox = np.array(
                    [[o2n[e[0]] + offset, o2n[e[1]] + offset] for e in edge_super_bbox])
                ...
                iou, ios = bbox_iou_ios_cpu(
                    proposal, gt_bbox[gt_bbox_idx_valid, :])
                ...
                if self.normalize_bbox:
                    ...
                    '''
                    pos_bbox = (pos_bbox - [min_x, min_y]) / \
                        [max_x - min_x, max_y - min_y]

                subcluster_slice_pos.append(
                    subcluster_slice_pos[-1] + pos_bbox.shape[0])
                subcluster_slice_edge.append(
                    subcluster_slice_edge[-1] + edge_bbox.shape[0])
                subcluster_slice_super.append(
                    subcluster_slice_super[-1] + edge_super_bbox.shape[0])
            ...
            root.value['idx_edge'] = (
                subcluster_slice_edge[idx_offset + max_idx], subcluster_slice_edge[idx_offset + max_idx + 1])
            root.value['idx_edge_super'] = (
                subcluster_slice_super[idx_offset + max_idx], subcluster_slice_super[idx_offset + max_idx + 1])
            ...
            for i in range(sub_bbox.shape[0]):
                ...
                p.value['idx_edge'] = (
                    subcluster_slice_edge[idx_offset + i], subcluster_slice_edge[idx_offset + i + 1])
                p.value['idx_edge_super'] = (
                    subcluster_slice_super[idx_offset + i], subcluster_slice_super[idx_offset + i + 1])
    ...
    def mixup(self, cc, pos, edge, edge_super, e_attr, e_attr_super, is_super):
        ...
        for cluster_i in range(len(cc)):
            ...
            is_super_merged = np.concatenate(
                [is_super[cluster], is_super[cluster_shuffled]], axis=0)
            ...
            edge_super_bb1 = update_edge_idx(
                edge_super_bb1, cluster_shuffled, idx_bb1)

            new_pos.append(pos_merged)
            new_cc.append(idx_merged)
            new_edge.append(np.concatenate([edge_bb0, edge_bb1], axis=0))
            new_edge_super.append(np.concatenate(
                [edge_super_bb0, edge_super_bb1, edge_super_merged], axis=0))
            new_e_attr.append(np.concatenate([e_attr_bb0, e_attr_bb1], axis=0))
            new_e_attr_super.append(np.zeros(
                (edge_super_bb0.shape[0] + edge_super_bb1.shape[0] + len(edge_super_merged), 6)))
        ...
        e_attr_super = np.concatenate(
            [e_attr_super] + new_e_attr_super, axis=0)
    ...
    def __getitem__(self, idx):
        ...
        if preload:
            try:
                pos, is_super, is_control, edge, edge_super, e_attr, e_attr_super, labels, bbox_idx, bbox, bbox_targets, stat_feats, has_obj, roots = pickle.load(
                    open(filename_bbox, 'rb'))
            except:
                # if not os.path.exists(filename_bbox):
                pos, is_super, is_control, edge, edge_super, e_attr, e_attr_super, labels, bbox_idx, bbox, bbox_targets, stat_feats, has_obj, roots = self._get_proposal(
                    graph_dict, gt_bbox, gt_labels, bbox_sampling_step=self.bbox_sampling_step)
                pickle.dump([pos, is_super, is_control, edge, edge_super, e_attr, e_attr_super, labels,
                            bbox_idx, bbox, bbox_targets, stat_feats, has_obj, roots], open(filename_bbox, 'wb'))
        else:
            pos, is_super, is_control, edge, edge_super, e_attr, e_attr_super, labels, bbox_idx, bbox, bbox_targets, stat_feats, has_obj, roots = self._get_proposal(
                graph_dict, gt_bbox, gt_labels, bbox_sampling_step=self.bbox_sampling_step)
            pickle.dump([pos, is_super, is_control, edge, edge_super, e_attr, e_attr_super, labels,
        ...
        if self.data_aug:
            pos, bbox, gt_bbox, bbox_targets = self.random_transfer(
                pos, bbox, gt_bbox, bbox_targets)
        ...
        if False:
            ...
            for bbox_i, (bbox_pos_idxs, bbox_edge, bbox_edge_super) in enumerate(zip(cc, cc_edge, cc_edge_super)):
                ...
                img = np.ones((math.ceil(height), math.ceil(width), 3)).astype(
                    np.uint8) * 255
                for e in bbox_edge:
                    cv2.line(img,
                             (int(pos[e[0], 0] * width),
                              int(pos[e[0], 1] * height)),
                             (int(pos[e[1], 0] * width),
                                 int(pos[e[1], 1] * height)),
                             (255, 0, 0),
                             5
                             )
                ...
                '''
                outname = os.path.dirname(filepath).split('/')[-1] + '_' + os.path.basename(
                    filepath).replace('.svg', '') + '_%d_label%d' % (bbox_i, labels[bbox_i]) + '.png'
