...
def mergeCluster(cc, bboxs, ratio=None, expand_length=None):
        ...
        for box in bboxs:
            ...
            if not expand_length:
                delta_min_x, delta_min_y, delta_max_x, delta_max_y = - \
                    ratio*x_len, - ratio*y_len, ratio*x_len, ratio*y_len
            elif not ratio:
                delta_min_x, delta_min_y, delta_max_x, delta_max_y = - \
                    expand_length[0], - \
                    expand_length[1], expand_length[0], expand_length[1]
            else:
                raise NotImplementedError('invalid expand')
            ...
            expand_bboxs.append(
                [expand_min_x, expand_min_y, expand_max_x, expand_max_y])
        return expand_bboxs
    ...
    for idx, box in enumerate(expand_bboxs):
        max_over_min_x = np.where(
            box[0] > expand_bboxs[:, 0], box[0], expand_bboxs[:, 0])
        min_over_max_x = np.where(
            box[2] > expand_bboxs[:, 2], expand_bboxs[:, 2], box[2])
        max_over_min_y = np.where(
            box[1] > expand_bboxs[:, 1], box[1], expand_bboxs[:, 1])
        min_over_max_y = np.where(
            box[3] > expand_bboxs[:, 3], expand_bboxs[:, 3], box[3])
        adj_matrix[idx] = np.logical_and(
            max_over_min_x <= min_over_max_x, max_over_min_y <= min_over_max_y)
    ...
    for cc_m in cc_merged:
        ...
        new_bboxs.append([min(cc_bboxs[:, 0]), min(
            cc_bboxs[:, 1]), max(cc_bboxs[:, 2]), max(cc_bboxs[:, 3])])
        ...
        for svg_name in svg_list:
            ...
            if len(cross_shape_edges) == 0:
                ...
                node_dict['edge']['super'] = np.concatenate(
                    [shape_shape_edges, cross_shape_edges], axis=0)
